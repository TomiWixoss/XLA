/**
 * Custom Hook for Steganography Extract with SSE Progress
 */
...
export function useExtractSSE() {
  ...
  const onSubmit = useCallback(async (formData: ExtractMessageInput) => {
    if (!stegoImage) return;
    
    const useDecryption = !!formData.password;
    ...
    try {
      ...
      formDataToSend.append('stego_image', stegoImage);
      formDataToSend.append('use_decryption', useDecryption.toString());
